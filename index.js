...
function normalizarModalidad(modalidad) {
  ...
  if (
    ...
    const error = new Error(
      "La modalidad debe ser E-LEARNING o PRESENCIAL."
    );
    error.codigo = 400;
    throw error;
  }
...
async function guardarResultado({
  ...
  try {
    ...
    const modalidad =
      normalizarModalidad(modalidadRecibida);
...
async function inicializarBase() {
  ...
  await agregarColumnaSiFalta(
    ...
    "modalidad",
    "ENUM('E-LEARNING','PRESENCIAL') NOT NULL DEFAULT 'E-LEARNING' AFTER `curso`"
  );
